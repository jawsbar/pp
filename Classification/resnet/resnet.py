...
def conv_layer(inpt, filter_shape, stride): # conv -> BN -> relu
    ...
    batch_norm = tf.layers.batch_normalization(conv, mean, var, beta, gamma, 0.001)
...
def residual_block(inpt, output_depth, down_sample, projection=False):
    ...
    if down_sample: # downsampling일 경우 stride 2
        filter_ = [1,2,2,1]
    ...
    if input_depth != output_depth: #피쳐맵 사이즈가 다를경우 zero padding 을 활용해서 증가시켜줌.
        if projection:

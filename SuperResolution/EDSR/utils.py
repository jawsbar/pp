...
def upsample(x,scale=2,features=64,activation=tf.nn.relu):
	...
	if scale == 2:
		...
		x = slim.conv2d(x,ps_features,[3,3],activation_fn=activation) # x = [?, 50, 50, 12] 로 줄임.
...
def PS(X, r, color=False):
	if color:
		Xc = tf.split(X, 3, 3)
		#  3채널로 구분하기 위해(rgb)
		X = tf.concat([_phase_shift(x, r) for x in Xc],3) # 최종 [?, imgsize x scale, imgsize x scale, 3]
	else:
		X = _phase_shift(X, r)
	return X

def _phase_shift(I, r): #업스케일 과정
	# l = [?, 50, 50, 4] ,  r = 2
	bsize, a, b, c = I.get_shape().as_list()
	bsize = tf.shape(I)[0]
	X = tf.reshape(I, (bsize, a, b, r, r)) # [bsize, a, b, 2, 2]
	X = tf.transpose(X, (0, 1, 2, 4, 3))   # 굳이 전환 시키는 이유는 값들을 섞기위함
										   # ex) ([1,1,1,1], [2,2,2,2]) -> ([1,2,1,2], [1,2,1,2])
	X = tf.split(X, a, 1)  # a, [bsize, b, r, r]
	X = tf.concat([tf.squeeze(x, axis=1) for x in X],2) # bsize, b, a*r, r
	X = tf.split(X, b, 1)  # b, [bsize, a*r, r]
	X = tf.concat([tf.squeeze(x, axis=1) for x in X],2)  # bsize, a*r, b*r
	# 최종적으로 r배 만큼의 height와 width를 증가 시킴(섞어서 합침)

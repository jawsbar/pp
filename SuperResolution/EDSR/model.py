...
class EDSR(object):
	...
	def predict(self,x):
		...
		if (len(x.shape) == 3) and not(x.shape[0] == self.img_size and x.shape[1] == self.img_size):
			num_across = x.shape[0]//self.img_size # 4
			num_down = x.shape[1]//self.img_size # 6
			#self.img_size = 50
			tmp_image = np.zeros([x.shape[0]*self.scale,x.shape[1]*self.scale,3]) # 240, 320 -> 480, 640 으로 뻥튀기
			...
			if (x.shape[0]%self.img_size != 0 and  x.shape[1]%self.img_size != 0):
				...
				# 음수로 처리하는경우는 나머지 삐져나온수를 처리하기위함. ex) 320 x 240의 경우 img_size 값이 50일때 20 x 40이 남는데 이 매트릭스에 대한 처리를 따로 시켜주는것. PS에 넣어야됨.

...
def arg_parser():
    ...
    parser.add_argument("--mode", default = 'testvid')
...
class CartoonGAN():
    ...
    def build_model(self):
        ...
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS) # test에서 BN을 사용하려면 배치단위의 moving average와 variance등을 알아야 하는데
                                                                # test에서는 배치단위로 진행되는게 아닌 각각의 것으로 진행이 될 수 있으므로
                                                                # training 당시의 moving average 와 moving variance등을 test시에 적용시키려고 쓰는것.
                                                                # 텐서플로우의 colloection 안에 moving average 와 variance의 업데이트된 값이 계속 저장됨.
                                                                # 그래서 test 시에도 배치단위가 아님에도 BN을 쓸 수 있는 것.
    ...
    def test_vid(self):
        ...
        while (True):
            ...
            ret, img = cap.read()
            images = cv2.resize(img, (self.image_size, self.image_size)) # 요거는 리사이즈해도 뜨는데.

            images = images/127.5-1
            images = np.reshape(images, [1, self.image_size, self.image_size, 3])
            vid = self.sess.run([self.fake_cartoon], feed_dict={self.photo_input: images,
                                                                   self.is_train: True})
            vid = np.squeeze(vid)
            vid = (vid + 1) * 127.5
            vid = vid.astype(np.uint8) # uint8로 변환해주는게 제일 중요함
            cv2.imshow('vid', vid)
